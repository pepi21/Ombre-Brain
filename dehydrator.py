...
【绝对不可违反的规则】：
- 专有名词必须原样保留：小汐、番茄、奶凍卷、尼莫、媽媽 等人名绝不可改写为「她」「对方」「用户」「他」
- 每个句子的主语必须明确写出，不允许省略
- 原文中用「」『』""或引号包住的直接引语，必须一字不差保留
- 两个不同人物的话或动作，绝不可合并进同一句
- 如果不确定某个人名是谁，保留原文写法
...
7. actor（动作发出者）：这段内容中，主要的动作/话语是谁发出的？填写人名。
   如果没有明确的人物动作，留空字符串。
8. target（动作接收者）：动作/话语的对象是谁？填写人名。
   如果没有明确的对象，留空字符串。
9. action（核心动作）：用一个动词概括主要动作，如"说"、"送"、"哭"、"写"。
   如果没有明确动作，留空字符串。

已知人物列表（优先从此列表匹配）：小汐、番茄、奶凍卷、尼莫、媽媽
...
# --- Canonical people list for subject validation ---
# --- 人物正规名列表，用于主语验证 ---
PEOPLE_CANONICAL_LIST = ["小汐", "番茄", "奶凍卷", "尼莫", "媽媽"]


def _extract_people_from_text(text: str) -> set[str]:
    """
    Extract known people names from text.
    从文本中提取已知人名。
    """
    found = set()
    for name in PEOPLE_CANONICAL_LIST:
        if name in text:
            found.add(name)
    return found
...
class Dehydrator:
    ...
    async def dehydrate(self, content: str, metadata: dict = None) -> str:
        ...
        # --- Validate people names in dehydrated result ---
        # --- 验证脱水结果中的人名完整性 ---
        validated_result = await self._validate_dehydration(content, result)

        # --- Cache the result ---
        self._set_cached_summary(content, validated_result)
        return self._format_output(validated_result, metadata)
    ...
    # ---------------------------------------------------------
    # Validation: check people name integrity after dehydration
    # 验证层：脱水后检查人名完整性
    # ---------------------------------------------------------
    async def _validate_dehydration(self, original: str, dehydrated: str, retries: int = 2) -> str:
        """
        Validate that dehydrated result preserves all people names from original.
        If validation fails, retry dehydration up to `retries` times.
        If all retries fail, return original content as-is (safe fallback).
        验证脱水结果保留了原文中的所有人名。
        失败则重试，全部失败则直接用原文（容错）。
        """
        original_people = _extract_people_from_text(original)
        if not original_people:
            # No known people in original, no validation needed
            return dehydrated

        dehydrated_people = _extract_people_from_text(dehydrated)

        # Check 1: original people must all appear in dehydrated
        missing = original_people - dehydrated_people
        # Check 2: dehydrated should not introduce new people not in original
        extra = dehydrated_people - original_people

        if not missing and not extra:
            return dehydrated  # Validation passed

        logger.warning(
            f"Dehydration validation failed / 脱水验证失败: "
            f"missing={missing}, extra={extra}. Retrying ({retries} left)..."
        )

        if retries <= 0:
            # All retries exhausted, use original content as fallback
            logger.warning("Dehydration validation exhausted retries, using original / 重试耗尽，使用原文")
            return original

        # Retry dehydration
        try:
            retry_result = await self._api_dehydrate(original)
            return await self._validate_dehydration(original, retry_result, retries - 1)
        except Exception as e:
            logger.warning(f"Dehydration retry failed: {e}, using original")
            return original
